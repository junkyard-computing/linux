...
static int trusty_probe(struct platform_device *pdev)
{
	...
	for_each_possible_cpu(cpu) {
		...
		/*
		 * kthread_create_on_cpu() already kthread_bind()s the thread to
		 * @cpu. kthread_set_per_cpu() (which additionally sets the
		 * internal KTHREAD_IS_PER_CPU flag) is not exported to modules;
		 * Trusty parks/unparks these nop threads from its own CPU-hotplug
		 * instance callbacks, so the flag is not required here.
		 */
		kthread_park(tw->nop_thread);
	}
