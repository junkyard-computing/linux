...
/*
 * Trusty identifies shared IPC buffers by an opaque buf_id, not by physical
 * address. We override the vdev's DMA ops so the virtio ring hands Trusty the
 * buf_id in place of a dma_addr. Kernel 6.11 replaced dma_map_ops.map_page with
 * .map_phys (phys_addr_t instead of page+offset); phys_to_virt recovers the
 * tipc_msg_buf, which was allocated from lowmem by the IPC driver.
 */
