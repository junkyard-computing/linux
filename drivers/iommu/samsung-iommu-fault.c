...
/*
 * Reduced (mainline) fault path: read the fault registers, print the fault
 * address/context, clear the interrupt and return. The AOSP fault-report and
 * debug-snapshot (dpm-policy/panic) machinery has been dropped for the mainline
 * bring-up.
 */
irqreturn_t samsung_sysmmu_irq_thread(int irq, void *dev_id)
{
	...
	sysmmu_get_fault_msg(drvdata, itype, vid, addr, is_secure,
			     fault_msg, sizeof(fault_msg));
	dev_err(drvdata->dev, "%s\n", fault_msg);

	sysmmu_clear_interrupt(drvdata, is_secure);
