...
static struct iommu_domain *samsung_sysmmu_domain_alloc_paging(struct device *dev)
{
	...
	/* mainline 7.2 moved pgsize_bitmap off iommu_ops onto the domain. */
...
static __maybe_unused void samsung_sysmmu_detach_dev(struct iommu_domain *dom,
						     struct device *dev)
{
	struct sysmmu_clientdata *client;
	struct samsung_sysmmu_domain *domain = to_sysmmu_domain(dom);
				    ...
				    int prot, gfp_t gfp, size_t *mapped)
{
	...
	while (pgcount--) {
		...
		sysmmu_pte_t *entry = section_entry(domain->page_table, iova);
		...
		if (pgsize == SECT_SIZE) {
			...
			if (IS_ERR(pent))
				...
				ret = lv2set_page(pent, paddr, pgsize, prot,
						  lv2entcnt);
		}
...
static struct iommu_group *samsung_sysmmu_device_group(struct device *dev)
{
	...
	/*
	 * The AOSP driver pre-attached a self-allocated unmanaged domain here for
	 * the `samsung,unmanaged-domain` case. That is illegal in the mainline 7.2
	 * IOMMU core during `device_group` (the group/device links aren't set up
	 * yet — it faults in __iommu_attach_group). Just return the group and let
	 * the core attach the default domain; the edgetpu driver maps its buffers
	 * into that domain itself (it issues no other DMA, so there is no IOVA
	 * allocator conflict).
	 */
	return group;
}
