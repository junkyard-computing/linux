...
/*
 * Mainline gs201 port note:
 *   The AOSP driver reached the PMU via exynos_pmu_update()/exynos_pmu_read()
 *   (<soc/google/exynos-pmu-if.h>). Mainline exposes the PMU as a regmap
 *   (exynos_get_pmu_regmap(), backed by the google,gs201-pmu driver which
 *   routes protected writes through the correct path). We fetch that regmap in
 *   probe and drive PHY isolation via regmap_update_bits()/regmap_read().
 *   Isolation is treated as non-fatal: the bootloader hands off the outer
 *   panel with the PHY already de-isolated (LCD_ON), so a missing PMU regmap
 *   must not block DSIM probe during bring-up.
 */
...
static int __set_phy_isolation(struct exynos_mipi_phy *state,
			       unsigned int offset, unsigned int on)
{
	unsigned int val;
	int ret;

	/* Non-fatal on mainline: bootloader hands off already de-isolated. */
	if (!state->pmu_regmap)
		return 0;

	val = on ? EXYNOS_MIPI_PHY_M4M4_ISO_BYPASS : 0;

	ret = regmap_update_bits(state->pmu_regmap, offset,
		...
		struct mipi_phy_desc *phy_desc, unsigned int on)
{
	...
	/* No PMU regmap: assume bootloader-initialised (LCD_ON) PHY. */
	if (!state->pmu_regmap) {
		phy_desc->data->flags |= MIPI_PHY_MxMx_INIT_DONE;
		return 0;
	}

	ret = regmap_read(state->pmu_regmap, phy_desc->iso_offset, &cfg);
		...
		struct mipi_phy_desc *phy_desc, unsigned int on)
{
	...
	if (on) {
		ret = __set_phy_isolation(state, phy_desc->iso_offset, on);
		if (ret)
			goto phy_exit;
...
static int exynos_mipi_phy_probe(struct platform_device *pdev)
{
	...
	phy_data = (struct exynos_mipi_phy_data *)of_device_get_match_data(dev);
	if (!phy_data)
		return -EINVAL;
	...
	/* PMU regmap for isolation control (optional on mainline: the panel is
	 * already de-isolated by the bootloader, so absence is non-fatal). */
	state->pmu_regmap = exynos_get_pmu_regmap();
	if (IS_ERR_OR_NULL(state->pmu_regmap)) {
		dev_info(dev, "no PMU regmap; PHY isolation control disabled\n");
		state->pmu_regmap = NULL;
	}
