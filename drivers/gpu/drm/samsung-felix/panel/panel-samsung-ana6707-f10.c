...
static int spanel_get_brightness(struct thermal_zone_device *tzd, int *temp)
{
	...
	/*
	 * struct thermal_zone_device is opaque to drivers in mainline; use the
	 * accessor rather than the AOSP-era ->devdata dereference.
	 */
...
static const struct thermal_zone_device_ops spanel_tzd_ops = {
...
static int ana6707_f10_panel_probe(struct mipi_dsi_device *dsi)
{
	...
	spanel->is_pixel_off = false;
	/*
	 * AOSP registered a 0-trip zone via thermal_zone_device_register(), which
	 * mainline removed; thermal_tripless_zone_device_register() is the direct
	 * equivalent. Note this "thermal zone" reports panel BRIGHTNESS, not
	 * temperature — it is an Android telemetry shim, so its failure must never
	 * block panel probe.
	 *
	 * Also fixes an AOSP bug: on registration failure it fell through and
	 * called thermal_zone_device_enable() on the ERR_PTR.
	 */
	spanel->tzd = thermal_tripless_zone_device_register("inner-disp", spanel,
							    &spanel_tzd_ops, NULL);
	if (IS_ERR(spanel->tzd)) {
		dev_err(&dsi->dev, "failed to register inner display thermal zone: %ld\n",
			PTR_ERR(spanel->tzd));
		spanel->tzd = NULL;
	} else {
		...
		if (ret) {
			dev_err(&dsi->dev,
				"failed to enable inner display thermal zone ret=%d\n", ret);
