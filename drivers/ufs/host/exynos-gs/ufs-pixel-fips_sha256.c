...
/*
 * This file is part of the ufs-pixel-fips kernel module, which is the software
 * part of a hybrid software/hardware FIPS cryptographic module that will
 * undergo FIPS 140-2 certification.  This certification requires that the
 * software part of the module do an integrity self-check using HMAC-SHA256,
 * using an HMAC-SHA256 implementation built into the module itself.  The module
 * also must be recertified any time its source code changes.  Therefore, this
 * file copies the SHA-256 implementation from lib/crypto/sha256.c in order to
 * incorporate it into the module while preventing future changes to that file
 * from invalidating the certification.
 */
...
static void ufs_pixel_fips_sha256_transform(u32 *state, const u8 *input)
{
				  ...
				  const u8 *data, unsigned int len)
{
	...
	if ((partial + len) > 63) {
		...
		do {
			ufs_pixel_fips_sha256_transform(sctx->state, src);
			done += 64;
			src = data + done;
		} while (done + 63 < len);
...
EXPORT_SYMBOL_GPL(ufs_pixel_fips_sha256_update);
				 ...
				 u8 *out)
{
	...
	ufs_pixel_fips_sha256_update(sctx, padding, pad_len);

	/* Append length (before padding) */
	ufs_pixel_fips_sha256_update(sctx, (const u8 *)&bits, sizeof(bits));
...
EXPORT_SYMBOL_GPL(ufs_pixel_fips_sha256_final);
