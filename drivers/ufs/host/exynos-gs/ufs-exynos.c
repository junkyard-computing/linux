...
#else
	return pixel_init(hba, ufs->dev, NULL);
#endif
}
...
/*
 * Mainline removed ufshcd_shutdown() and now handles shutdown via the
 * platform-driver .remove + device-tree shutdown ordering. Drop the
 * explicit .shutdown callback to match.
 */
...
static const struct of_device_id exynos_ufs_match[] = {
	...
	/*
	 * felix DTS (gs201.dtsi) uses "google,gs201-ufs"; mainline's
	 * drivers/ufs/host/ufs-exynos.c also matches "google,gs101-ufs". With
	 * SCSI_UFS_EXYNOS=n in our config, mainline's table is gone, so we
	 * pick up these compats here. The per-SoC behavior is gated by the
	 * cal-if files compiled in (drivers/ufs/host/exynos-gs/{gs101,gs201,
	 * zuma}/), which are selected by CONFIG_SOC_GS{101,201}/CONFIG_SOC_ZUMA.
	 */
