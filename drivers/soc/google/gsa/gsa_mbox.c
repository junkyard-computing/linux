				     ...
				     struct gsa_mbox_rsp *rsp)
{
	...
	/*
	 * Wait for the response IRQ, but bounded: some commands are not serviced
	 * by GSA firmware in every configuration (e.g. WAKELOCK_ACQUIRE without
	 * the full pKVM/s2mpu setup) and never raise the response interrupt. An
	 * unbounded wait_for_completion() here wedges the calling thread (and any
	 * userspace behind it) in-kernel with no way to recover but a reboot.
	 */
	if (!wait_for_completion_timeout(&mb->mbox_cmd_completion,
					 msecs_to_jiffies(5000))) {
		dev_err(mb->dev, "mbox cmd=0x%x timed out (no response irq)\n",
			req->cmd);
		ret = -ETIMEDOUT;
		goto err_bad_rsp;
	}
...
/*
 * Without CONFIG_GSA_PKVM there is no s2mpu device to power-manage. Tried
 * sending GSA_MB_CMD_WAKELOCK_ACQUIRE here to power GSACORE's SYSMMU_S2 (the
 * "running under Hypervisor" path above), but felix's GSA firmware never
 * services that command in this configuration — the response IRQ never fires
 * and the mailbox wait hangs. So the wakelock is NOT the fix for the
 * LOAD_TPU_FW_IMG INVALID_ARGS; keep the data-xfer prepare a no-op.
 */
static int gsa_data_xfer_prepare_locked(struct gsa_mbox *mb)
{
	return 0;
