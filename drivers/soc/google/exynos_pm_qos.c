...
/*
 * AOSP's <trace/events/power.h> declares a clock_set_rate tracepoint that
 * mainline doesn't carry. Stub trace_clock_set_rate to a no-op; this is pure
 * observability for power-related debug.
 */
#define trace_clock_set_rate(name, val, cpu) do { (void)(name); (void)(val); (void)(cpu); } while (0)
