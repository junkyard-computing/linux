...
/*
 * set_priv_reg / rmw_priv_reg normally live in AOSP's exynos-pd_el3.c and
 * issue an SMC to EL3 to write protected PMU registers. Mainline doesn't
 * have that EL3 monitor port, and the only consumer that calls these in our
 * graft is exynos_pmu_write / exynos_pmu_*_bit_atomic — none of which the
 * vendor UFS driver actually invokes (it uses exynos_pmu_read). Provide weak
 * stubs so the link succeeds; if something ever does call the write path
 * we'll get a no-op rather than a crash, which is the safer failure mode
 * during early bring-up.
 */
__weak int set_priv_reg(phys_addr_t reg, u32 val)
{
	return 0;
}

__weak int rmw_priv_reg(phys_addr_t reg, u32 mask, u32 val)
{
	return 0;
}
